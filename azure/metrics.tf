...
locals {
  monitoring_name = "MSProm-${azurerm_resource_group.infra.location}-${azurerm_kubernetes_cluster.cluster.name}"

}

resource "azurerm_monitor_data_collection_endpoint" "dce" {
  name = substr(local.monitoring_name, 0, min(44, length(local.monitoring_name)))
  resource_group_name = azurerm_resource_group.infra.name
  location            = azurerm_resource_group.infra.location
  kind                = "Linux"
}
...
resource "azurerm_monitor_data_collection_rule" "dcr" {
  name = substr(local.monitoring_name, 0, min(64, length(local.monitoring_name)))
  resource_group_name         = azurerm_resource_group.infra.name
  location                    = azurerm_resource_group.infra.location
  ...
  destinations {
    monitor_account {
      monitor_account_id = azurerm_monitor_workspace.monitoring.id
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  name                = "NodeRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.cluster.name}"
  location            = azurerm_resource_group.infra.location
  resource_group_name = azurerm_resource_group.infra.name
  ...
  interval            = "PT1M"
  scopes = [azurerm_monitor_workspace.monitoring.id, azurerm_kubernetes_cluster.cluster.id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules_rule_group" {
  name                = "KubernetesRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.cluster.name}"
  location            = azurerm_resource_group.infra.location
  resource_group_name = azurerm_resource_group.infra.name
  cluster_name        = azurerm_kubernetes_cluster.cluster.name
